...
func testArticlesExists(t *testing.T) {
	...
	e, err := ArticleExists(tx, article.IDArticles)
	if err != nil {
...
func testArticlesFind(t *testing.T) {
	...
	articleFound, err := FindArticle(tx, article.IDArticles)
	if err != nil {
		t.Error(err)
	}
...
func testArticlesInsertWhitelist(t *testing.T) {
	...
	tx := MustTx(boil.Begin())
	defer tx.Rollback()
	if err = article.Insert(tx, articleColumns...); err != nil {
		t.Error(err)
	}
...
var (
	articleDBTypes = map[string]string{`Content`: `longtext`, `CreatedAt`: `datetime`, `IDArticles`: `int`, `Thumbnail`: `varchar`, `Title`: `varchar`, `UpdatedAt`: `datetime`}
	_              = bytes.MinRead
)
